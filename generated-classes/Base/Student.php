...
abstract class Student implements ActiveRecordInterface
{
     ...
     *
     * @return Student The current object, for fluid interface
     */
    ...
    public function setUserId($v)
    {
        ...
        if ($this->user_id !== $v) {
            $this->user_id = $v;
            $this->modifiedColumns[] = StudentTableMap::USER_ID;
        }
    ...
    public function setGroupId($v)
    {
        ...
        if ($this->group_id !== $v) {
            $this->group_id = $v;
            $this->modifiedColumns[] = StudentTableMap::GROUP_ID;
        }
    ...
    public function setDeptId($v)
    {
        ...
        if ($this->dept_id !== $v) {
            $this->dept_id = $v;
            $this->modifiedColumns[] = StudentTableMap::DEPT_ID;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : StudentTableMap::translateFieldName('UserId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->user_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : StudentTableMap::translateFieldName('GroupId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->group_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : StudentTableMap::translateFieldName('DeptId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = StudentTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException("Error populating \Student object", 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(StudentTableMap::USER_ID)) {
            $modifiedColumns[':p' . $index++]  = 'USER_ID';
        }
        if ($this->isColumnModified(StudentTableMap::GROUP_ID)) {
            $modifiedColumns[':p' . $index++]  = 'GROUP_ID';
        }
        if ($this->isColumnModified(StudentTableMap::DEPT_ID)) {
            $modifiedColumns[':p' . $index++]  = 'DEPT_ID';
        }

        $sql = sprintf(
            'INSERT INTO student_tbl (%s) VALUES (%s)',
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(StudentTableMap::USER_ID)) $criteria->add(StudentTableMap::USER_ID, $this->user_id);
        if ($this->isColumnModified(StudentTableMap::GROUP_ID)) $criteria->add(StudentTableMap::GROUP_ID, $this->group_id);
        if ($this->isColumnModified(StudentTableMap::DEPT_ID)) $criteria->add(StudentTableMap::DEPT_ID, $this->dept_id);
    ...
    {
        $criteria = new Criteria(StudentTableMap::DATABASE_NAME);
        $criteria->add(StudentTableMap::USER_ID, $this->user_id);
    ...
    public function getDepartment(ConnectionInterface $con = null)
    {
        if ($this->aDepartment === null && ($this->dept_id !== null)) {
            ...
            /* The following can be used additionally to
                ...
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aDepartment->addStudents($this);
             */
        }
